...
export default function GaokaoSubjectPage({
  ...
  useEffect(() => {
    ...
    async function loadOne(type: Mode) {
      const res = await fetch(`/api/highschool/gaokao/${subject}?type=${type}`, {
        signal: ctrl.signal,
        cache: "no-store",
      });
  ...
  if (!config) { router.replace("/highschool/gaokao"); return null; }
  ...
  async function generate(type: Mode) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/highschool/gaokao/${subject}`, {
        method: "POST",
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-8">
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        <Link href="/highschool" className="text-[#9a9590] hover:text-[#5a5550] text-[13px]">高中学习</Link>
        <span className="text-[#d8d4ca]">/</span>
        <Link href="/highschool/gaokao" className="text-[#9a9590] hover:text-[#5a5550] text-[13px]">高考专题</Link>
        <span className="text-[#d8d4ca]">/</span>
        ...
        <>
          ...
          <div className="flex items-center justify-between mb-5 px-4 py-3 border border-[#e4e0d8] bg-white" style={{ borderLeftWidth: 3, borderLeftColor: accent }}>
            <div>
              <p className="text-[13px] font-bold text-[#1c1a16]" style={{ fontFamily: "var(--font-playfair, Georgia, serif)" }}>
                {mode === "real" ? `${name}高考真题仿真练习` : `${name} 2026 高考预测卷`}
              </p>
          ...
          {!currentLoading && currentContent && (
            <div className="space-y-4">
              <style>{`
                .gk-md h2 { font-size:1rem;font-weight:700;margin-top:0.6rem;margin-bottom:0.5rem;color:${accent};font-family:var(--font-playfair,Georgia,serif); }
                .gk-md h1 { font-size:1.15rem;font-weight:700;margin-bottom:0.6rem;color:${accent};font-family:var(--font-playfair,Georgia,serif); }
                .gk-md p { font-size:0.875rem;line-height:1.75;color:#1c1a16;margin-bottom:0.55rem; }
                .gk-md ul,.gk-md ol { font-size:0.875rem;padding-left:1.25rem;color:#1c1a16; }
                .gk-md li { margin-bottom:0.3rem;line-height:1.65; }
                .gk-md strong { font-weight:700;color:#1c1a16; }
                .gk-md blockquote { border-left:3px solid ${accent};padding:0.5rem 0.85rem;background:#faf8f5;color:#5a5550;font-size:0.85rem;margin:0.5rem 0;font-style:italic; }
                .gk-md hr { border-color:#e4e0d8;margin:0.85rem 0; }
                .gk-md code { background:#f5f2eb;padding:0.05rem 0.35rem;border-radius:2px;font-size:0.8rem; }
              `}</style>

              {/* Intro / trend analysis */}
              {parsed.intro && (
                  ...
                  <div key={key} className="border border-[#d8d4ca] bg-white">
                    ...
                    {q.answer && (
                      <div className="border-t border-[#e4e0d8]">
                        ...
                        {open && (
                          <div className="px-5 pb-4 pt-1 bg-[#faf8f5] gk-md">
                            <ReactMarkdown
                              remarkPlugins={enableMath ? [remarkGfm, remarkMath] : [remarkGfm]}
                              rehypePlugins={enableMath ? [rehypeKatex] : []}
                            >
